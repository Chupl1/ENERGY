...
# ==============================================================================
# --- BARRE LATÉRALE COMMUNE (CORRIGÉE DE MANIÈRE NATIVE STREAMLIT) ---
# ==============================================================================
st.sidebar.header("🎯 Filtres Globaux")

# 1. Initialisation propre des valeurs par défaut dans session_state si elles n'existent pas
...
# 2. Widgets reliés directement à session_state grâce au paramètre `key`
annee_choisie = st.sidebar.selectbox("Sélectionnez l'année", liste_annees, key='annee')

mois_choisi_num = st.sidebar.selectbox(
    "Sélectionnez le mois", 
    options=list(dict_mois.keys()), 
    format_func=lambda x: dict_mois[x],
    key='mois_num'
)

region_choisie = st.sidebar.selectbox("Sélectionnez la Région", liste_regions, key='region')

# Mettre à jour le nom du mois dans la session
...
with tab_kpi:
    ...
    with col_v2:
        st.metric(
            ...
            value=f"{puissance_conso_moyenne:,.0f}".replace(',', ' '),
    ...
    with col_m1:
        st.metric(
            label="⚛️ Part du Nucléaire dans la Production", 
            value=f"{(prod_nucleaire/prod_totale)*100:.0f} %" if prod_totale > 0 else "0 %"
        )
    with col_m2:
        st.metric(
            label="🌱 Part des Énergies Renouvelables", 
            value=f"{(prod_renouvelable/prod_totale)*100:.0f} %" if prod_totale > 0 else "0 %"
        )
        
    st.write("---")
    
    # ==============================================================================
    # 📈 ANALYSE MACRO-HISTORIQUE
    # ==============================================================================
